...
document.getElementById('concept-btn').addEventListener('click', async (event) => {
  await run();
  console.log('return from gemini');
  openAiPopup(resume_rep);

});


function openAiPopup(response) {
    var blurOverlay = document.getElementById('blur-overlay');
    blurOverlay.style.display = 'block';
    var popup = document.createElement('div');
    popup.id = 'aipopup';
    popup.innerHTML =    `
    <div class="shower-ai glassify">
      <span class="bigwr littlebottom">Summary Of Your Thoughts</span>
      <span class="normalwr">${response}</span>
    </div>
    `;
